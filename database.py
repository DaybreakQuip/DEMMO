...
class GameDatabase:
	@classmethod
	def create_game_object_tables(cls):
		'''
		Player table:
			coordinates: (x,y)
			id: #
	...
	@classmethod
	def update_player(cls, player_id, coordinates, health, power, gold, num_bosses_defeated):
		conn = sqlite3.connect(game_db)  # connect to that database (will create if it doesn't already exist)
		c = conn.cursor()  # make cursor into database (allows us to execute commands)
	...
	@classmethod
	def get_monster_info(cls, coordinates):
		conn = sqlite3.connect(game_db)  # connect to that database (will create if it doesn't already exist)
		c = conn.cursor()  # make cursor into database (allows us to execute commands)
